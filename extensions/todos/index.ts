...
interface TodoSelectorComponent extends RuntimeComponentLike {}
interface TodoActionMenuComponent extends RuntimeComponentLike {}
interface TodoDeleteConfirmComponent extends RuntimeComponentLike {}
...
class TodoSelectorComponent extends Container implements Focusable {
  ...
  private listContainer: RuntimeComponentLike;
  ...
  constructor(
    ...
    this.addChild(new Spacer(1));
    this.listContainer = new Container() as unknown as RuntimeComponentLike;
...
async function deleteTodo(
  ...
  const result = await withTodoLock(todosDir, normalizedId, ctx, async () => {
    ...
    if (!existing)
      return { error: `Todo ${displayTodoId(id)} not found` } as const;
    if (!options.allowOpen && !isTodoClosed(existing.status)) {
      return {
        error: `Todo ${displayTodoId(id)} is open; close it before deleting or use /todos for interactive confirmed deletion.`,
      } as const;
    }
    await fs.unlink(filePath);
    return existing;
...
export default function todosExtension(pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    description:
      ...
      "Claim tasks before working on them to avoid conflicts, close them when complete, and only delete closed todos from the tool.",
    promptSnippet:
      `Manage file-based todos in ${todosDirLabel} (list, list-all, get, create, update, append, delete, claim, release). ` +
      "Title is the short summary; body is long-form markdown notes (update replaces, append adds). " +
      "Todo ids are shown as TODO-<hex>; id parameters accept TODO-<hex> or the raw hex filename. " +
    ...
    handler: async (args, ctx) => {
      ...
      await ctx.ui.custom((tui, theme, _kb, done) => {
        ...
        const openTodoOverlay = async (
          ...
          const action: TodoOverlayAction | null = await ctx.ui.custom(
