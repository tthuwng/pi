...
import { installAuthStorage } from "../lib/auth-compat.mjs";
...
function buildNativeOAuth(
	provider: string,
	index: number,
	displayName: string,
): Omit<OAuthProviderInterface, "id"> {
	const native = getNativeOAuth(provider);
	const name = `${displayName} #${index}`;
	if (!native) {
		return {
			name,
			async login() {
				throw new Error(`${provider} OAuth is not available in this Pi runtime`);
			},
			async refreshToken() {
				throw new Error(`${provider} OAuth is not available in this Pi runtime`);
			},
	...
	return createLegacyOAuthAdapter(native, { name });
}

function getNativeOAuth(provider: string) {
	return builtinProviders().find((candidate) => candidate.id === provider)?.auth.oauth;
}

async function refreshNativeOAuth(provider: string, credentials: OAuthCredentials) {
	const native = getNativeOAuth(provider);
	if (!native) throw new Error(`${provider} OAuth is not available in this Pi runtime`);
	return native.refresh(credentials, new AbortController().signal);
}
...
const PROVIDER_TEMPLATES: Record<string, ProviderTemplate> = {
	anthropic: {
		...
		buildOAuth(index: number) {
			return buildNativeOAuth("anthropic", index, "Anthropic");
		},
	},
	...
	"openai-codex": {
		...
		buildOAuth(index: number) {
			return buildNativeOAuth("openai-codex", index, "ChatGPT Codex");
		},
	},
	...
	"github-copilot": {
		...
		buildOAuth(index: number) {
			return buildNativeOAuth("github-copilot", index, "GitHub Copilot");
		},
	},
	...
	"google-gemini-cli": {
		...
		buildOAuth(index: number) {
			return buildNativeOAuth("google-gemini-cli", index, "Google Cloud Code Assist");
		},
	},
	...
	"google-antigravity": {
		...
		buildOAuth(index: number) {
			return buildNativeOAuth("google-antigravity", index, "Antigravity");
		},
	},
};
...
async function resolveGoogleQuotaAccess(
	...
	if (typeof auth.refresh === "string" && auth.refresh.length > 0) {
		const credentials = account.baseProvider === "google-gemini-cli"
			? await refreshNativeOAuth("google-gemini-cli", auth as GeminiCredentials)
			: await refreshNativeOAuth("google-antigravity", auth as GeminiCredentials);
		return {
...
function formatCodexSessionStatus({
	mode,
	account,
	session,
}: {
...
function getStatusMode(ctx: ExtensionContext): "fast" | "normal" {
	return ctx.thinkingLevel && ctx.thinkingLevel !== "off" ? "normal" : "fast";
}

function getShortProviderName(providerName: string): string {
	const baseProvider = getBaseProvider(providerName) || providerName;
	if (baseProvider === "openai-codex") return "Codex";
	if (baseProvider === "anthropic") return "Claude";
	if (baseProvider === "github-copilot") return "Copilot";
	return PROVIDER_TEMPLATES[baseProvider]?.displayName || baseProvider;
}

function getActiveAccountLabel(ctx: ExtensionContext, effective: EffectiveConfig): string {
	const providerName = ctx.model?.provider || "unknown";
	const subscription = effective.subscriptions.find((entry) => subProviderName(entry) === providerName);
	if (subscription) {
		const label = subscription.label ? `${subscription.label} · ` : "";
		return `${label}${getShortProviderName(subscription.provider)} #${subscription.index}`;
	}
	return getShortProviderName(providerName);
}

function publishCodexStatus(ctx: ExtensionContext): void {
	const effective = loadEffectiveConfig(ctx.cwd);
	ctx.ui.setStatus(
		"multi-pass",
		formatCodexSessionStatus({
			mode: getStatusMode(ctx),
			account: getActiveAccountLabel(ctx, effective),
			session: ctx.sessionManager.getSessionName() || "unnamed",
		}),
	);
	// pi-lens keeps its own diagnostic status useful in the UI, but it should
	// not displace the account/session/quota status in the compact footer.
