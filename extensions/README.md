...
implementations. Pi loads the selected extensions from this directory.
...
The root `pi/settings.json` loads selected extensions with this manifest:

```json
{
  "source": "./extensions",
  "extensions": ["multi-pass/extensions/multi-sub.ts"]
}
```
...
Use the generic example in [`multi-accounts/multi-pass.example.json`](multi-accounts/multi-pass.example.json)
