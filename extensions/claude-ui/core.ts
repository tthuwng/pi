...
const MAX_EXTERNAL_DIFFS = 10;
const EXTERNAL_DIFF_MAX_BUFFER = 5 * 1024 * 1024;
const EXTERNAL_DIFF_TIMEOUT_MS = 5_000;
const MAX_EXTERNAL_DIFF_INPUT_CHARS = 200_000;
const MAX_EXTERNAL_DIFF_OUTPUT_CHARS = 250_000;
const MAX_EXTERNAL_DIFF_CACHE_CHARS = 1_000_000;
const MAX_INLINE_WRITE_DIFF_CHARS = 200_000;
const MAX_INLINE_WRITE_DIFF_LINES = 1_500;
const MAX_DIFF_SOURCE_BYTES = MAX_EXTERNAL_DIFF_INPUT_CHARS;
...
function forgetExternalDiff(toolCallId: string): void {
	externalDiffCacheChars -= forgetStringEntry(externalDiffs, toolCallId);
}

function clearExternalDiffs(): void {
	externalDiffs.clear();
	externalDiffCacheChars = 0;
}

function rememberExternalDiff(toolCallId: string, diff: string): void {
	if (diff.length > MAX_EXTERNAL_DIFF_OUTPUT_CHARS) return;
	externalDiffCacheChars = rememberBoundedStringEntry(
		externalDiffs,
		toolCallId,
		diff,
		externalDiffCacheChars,
		MAX_EXTERNAL_DIFFS,
		MAX_EXTERNAL_DIFF_CACHE_CHARS,
	);
}

function exceedsLineLimit(value: string, maxLines: number): boolean {
	let lines = 1;
...
function isDiffInputBounded(
	...
	return (
		oldContent.length + newContent.length <= maxChars &&
		!exceedsLineLimit(oldContent, maxLines) &&
		!exceedsLineLimit(newContent, maxLines)
	);
}

function omittedDiffSummary(oldContent: string, newContent: string): string {
	return `Diff omitted: ${formatTokenCount(oldContent.length + newContent.length)} chars across ${plural(countLines(oldContent) + countLines(newContent), "line")} exceeds claude-ui render limit.`;
}
...
function persistExternalDiff(
	...
	// External diffs remain attached to the current tool result; persisting every
	// diff bloats session logs and slows resume without affecting LLM context.
	void pi;
	void toolCallId;
	void diff;
}

function restoreExternalDiffs(_ctx: ExtensionContext): void {
	// New external diffs are intentionally not persisted. Avoid scanning old,
	// bloated sessions and rehydrating historical diff payloads on startup.
	clearExternalDiffs();
}
...
async function renderExternalDiff(
	...
	if (
		!isDiffInputBounded(
			oldContent,
			newContent,
			MAX_EXTERNAL_DIFF_INPUT_CHARS,
			MAX_INLINE_WRITE_DIFF_LINES,
		)
	) {
		return undefined;
	}
...
`;
		const { stdout } = await execFileAsync(
			...
			["-c", script, "claude-ui-diff", oldPath, newPath, displayPath],
			{
				...
				timeout: EXTERNAL_DIFF_TIMEOUT_MS,
			},
		);
		return stdout.length > 0 && stdout.length <= MAX_EXTERNAL_DIFF_OUTPUT_CHARS
			? stdout
			: undefined;
	} finally {
...
async function readBoundedTextFile(path: string): Promise<BoundedTextFile> {
	try {
		const fileStat = await stat(path);
		if (!fileStat.isFile()) {
			return { existed: true, error: "not a regular file" };
		}
		if (fileStat.size > MAX_DIFF_SOURCE_BYTES) {
			return {
				existed: true,
				error: `file is ${formatTokenCount(fileStat.size)} bytes; diff capture limit is ${formatTokenCount(MAX_DIFF_SOURCE_BYTES)} bytes`,
			};
		}
		...
		return { existed: true, error: errorMessage(error) };
	}
}
...
function renderEditResult(
	...
	const renderedDiff = externalDiffs.get(context.toolCallId) ?? diff;
...
function renderWriteResult(
	...
	const canGenerateInlineDiff =
		previousContent !== undefined &&
		isDiffInputBounded(
			previousContent,
			content,
			MAX_INLINE_WRITE_DIFF_CHARS,
			MAX_INLINE_WRITE_DIFF_LINES,
		);
	const diff =
		previousContent === undefined
			? ""
			: canGenerateInlineDiff
				? generateDiffString(previousContent, content)
				: omittedDiffSummary(previousContent, content);
	const renderedDiff = canGenerateInlineDiff
		? (externalDiffs.get(context.toolCallId) ?? diff)
		: diff;
	const { additions, removals } = canGenerateInlineDiff
		? countDiffLines(diff)
		: { additions: 0, removals: 0 };
	...
	const warning = snapshot?.captureError
		...
		: previousContent === undefined
			...
			: canGenerateInlineDiff
				? undefined
				: "diff omitted because it is too large to render safely";
	const diffSummary = canGenerateInlineDiff
		? ` · +${additions} -${removals}`
		: previousContent === undefined
			? " · diff unavailable"
			: " · diff omitted";
		...
		execute: async (toolCallId, params, signal, onUpdate, ctx) => {
			...
			const absolutePath =
				typeof editPath === "string" && editPath !== ""
					...
					: undefined;
			const oldFile = absolutePath
				? await readBoundedTextFile(absolutePath)
				: undefined;
			...
			if (
				...
				try {
					const newFile = await readBoundedTextFile(absolutePath);
					if (newFile.content === undefined) return result;
		...
		execute: async (toolCallId, params, signal, onUpdate, ctx) => {
			...
			if (typeof content === "string" && typeof displayPath === "string") {
				try {
					...
					if (snapshot?.previousContent === undefined) return result;
					const diff = await renderExternalDiff(
