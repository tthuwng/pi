...
const OutputOverride = Type.Unsafe({
	...
	description:
		"Output filename/path (string), or false to disable file output. Relative path resolution depends on execution mode: top-level tasks resolve against cwd or task cwd; foreground chain steps resolve under the chain artifact directory; background chains resolve against cwd or step cwd.",
});
...
const ChainItem = Type.Object(
	...
	{
		description:
			"Chain step: use {agent, task?, ...} for sequential or {parallel: [...]} for concurrent execution",
	},
);
...
const ControlOverrides = Type.Object({
	enabled: Type.Optional(
		Type.Boolean({
			description:
				"Enable/disable subagent control attention tracking for this run",
		}),
	),
	...
	activeNoticeAfterMs: Type.Optional(
		Type.Integer({
			minimum: 1,
			description:
				"Active-long-running notice threshold by elapsed ms (default: 240000)",
		}),
	),
	...
	notifyOn: Type.Optional(
		Type.Array(
			Type.String({ enum: ["active_long_running", "needs_attention"] }),
			{
				description:
...
export const SubagentParams = Type.Object({
	agent: Type.Optional(
		Type.String({
			description:
				"Agent name (SINGLE mode) or target for management get/update/delete",
		}),
	),
	task: Type.Optional(
		Type.String({
	...
	config: Type.Optional(
		Type.Unsafe({
			...
			description:
				"Agent or chain config for create/update. Agent: name, package (optional namespace; runtime name becomes package.name), description, scope ('user'|'project', default 'user'), systemPrompt, systemPromptMode, inheritProjectContext, inheritSkills, defaultContext ('fresh'|'fork'), model, tools (comma-separated), extensions (comma-separated), skills (comma-separated), thinking, output, reads, progress, maxSubagentDepth. Chain: name, package, description, scope, steps (array of {agent, task?, output?, outputMode?, reads?, model?, skills?, progress?}). Presence of 'steps' creates a chain instead of an agent. String values must be valid JSON.",
		}),
	),
	tasks: Type.Optional(
		Type.Array(TaskItem, {
			description:
				"PARALLEL mode: [{agent, task, count?, output?, outputMode?, reads?, progress?}, ...]. Relative tasks[*].output paths resolve against cwd or each task cwd; if outputMode is file-only, inspect/read the saved artifact before synthesis.",
		}),
	),
	concurrency: Type.Optional(
		Type.Integer({
	...
	chainDir: Type.Optional(
		Type.String({
			description:
				"Persistent directory for chain artifacts. Default: a user-scoped temp directory under <tmpdir>/; stale default chain directories older than 24h are cleaned up best-effort on extension startup.",
		}),
	),
	async: Type.Optional(
		Type.Boolean({
	...
	clarify: Type.Optional(
		Type.Boolean({
			description:
				"Show TUI to preview/edit before execution (default: false; opt in with true). Implies sync mode.",
		}),
	),
